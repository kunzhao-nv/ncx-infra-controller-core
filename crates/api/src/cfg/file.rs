...
pub struct CarbideConfig {
    ...
    /// clients and forge-admin-cli to connect.
    /// Default is `[::]:1079`.
    ...
    /// when running a second carbide instance against a
    ...
    /// Used by forge-dpu-agent to write `frr.conf` for
    /// BGP routing.
    pub asn: u32,
    ...
    /// GNI-provided L3 VNI for FNN VPC Internet
    ...
    /// Forge with a VM system, including VF settings and
...
pub struct AdminFnnConfig {
    ...
    /// VNI for the admin network VPC. When enabled, carbide will create a VPC with this VNI
...
pub struct MachineValidationTestConfig {
    /// Unique test identifier (e.g., "forge_MmMemLatency").
    pub id: String,

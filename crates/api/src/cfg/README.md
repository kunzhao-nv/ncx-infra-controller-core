...
This document describes every section and field in the `carbide-api-config.toml`
...
| `asn` | `u32` | **required** | Autonomous System Number, fixed per environment. Used by forge-dpu-agent for `frr.conf` BGP routing. |
...
| `initial_dpu_agent_upgrade_policy` | `Option<AgentUpgradePolicyChoice>` | — | Policy for forge-dpu-agent upgrades. Also settable via `forge-admin-cli`. |
...
| `internet_l3_vni` | `u32` | `100001` | GNI-provided L3 VNI for FNN VPC Internet connectivity. Combined with `datacenter_asn` for route-target. |
...
| `cli_certs` | `Option<AllowedCertCriteria>` | — | Additional allowed cert criteria for forge-admin-cli. |
...
| `common_internal_route_target` | `Option<RouteTargetConfig>` | — | Double-tag for internal tenant routes (consumed by GNI). |
...
| `mqtt_endpoint` | `String` | `"mqtt.forge"` | MQTT broker host for DPA. |
...
| `mqtt_endpoint` | `String` | `"mqtt.forge"` | MQTT broker host. |
| `mqtt_broker_port` | `u16` | `1884` | MQTT broker port. |

...
func TestManageExpectedRackGroup_UpdateExpectedRackGroupsInDB(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "Core Switch is stored as REST NVSwitch", inventory: &corev1.ExpectedRackGroupInventory{InventoryStatus: corev1.InventoryStatus_INVENTORY_STATUS_SUCCESS, ExpectedRackGroups: []*corev1.ExpectedRackGroup{{Topology: "topology", Members: []*corev1.ExpectedRackGroupMember{{Type: "Switch", Manufacturer: "NVIDIA", Id: "device-01"}}}}}, wantType: cdbm.ExpectedRackGroupMemberTypeNVSwitch},
		{name: "invalid member does not delete inventory", inventory: &corev1.ExpectedRackGroupInventory{InventoryStatus: corev1.InventoryStatus_INVENTORY_STATUS_SUCCESS, ExpectedRackGroups: []*corev1.ExpectedRackGroup{{Topology: "topology", Members: []*corev1.ExpectedRackGroupMember{{Type: "NVSwitch", Manufacturer: "NVIDIA", Id: "device-01"}}}}}, wantErr: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.deleted {
				...
				require.NoError(t, err)
				if tc.wantType != "" {
					require.Equal(t, cdbm.ExpectedRackGroupMembers{{Type: tc.wantType, Manufacturer: "NVIDIA", ID: "device-01"}}, stored.Members)
				}
